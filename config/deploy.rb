...
namespace :vlad do
    ...
    remote_task :before_update, :roles => :app do
      run "tar zcf backup_before_update_#{Date.today.year}#{Date.today.month}#{Date.today.day}.tar #{deploy_to}"
    end
    ...
    remote_task :copy_data_files, :roles => :app do
      ...
      run "cp -r #{releases_path}/#{releases[releases.size - 2]}/public/site_doc #{current_path}/public"
    end
