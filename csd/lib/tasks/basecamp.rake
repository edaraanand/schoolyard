...
namespace :basecamp do
  ...
  task :notify_new_build do
    git_log = `git-log --abbrev-commit --pretty=format:"%s%n<a href=https://github.com/bjbolz/schoolapp/commit/%H target=_blank>%h</a> by (%an: %ar)%n" f6d007ab..HEAD --no-merges`
    build_msg = <<-EOF
